...
public class FairyJob {
  ...
  private static final String[] MCP_FLEEINGTICK = {"fleeingTick",
                                                   "field_70788_c"};
  ...
  public FairyJob(final EntityFairy entityfairy) { fairy = entityfairy; }

  public void discover(final World world) {
    if (fairy.getEntityToAttack() != null || fairy.getEntityFear() != null ||
        fairy.getCryTime() > 0 || fairy.attackTime > 0 ||
        fairy.getHealth() <= 0) {
      return;
    }

    final int x = MathHelper.floor(fairy.posX);
    int y = MathHelper.floor(fairy.getEntityBoundingBox().minY);

    if (fairy.flymode()) {
      y--;
    }

    final int z = MathHelper.floor(fairy.posZ);
  ...
  public ArrayList<EntityItem> getGoodies(final World world) {
    final List<?> list = world.getEntitiesWithinAABB(
        EntityItem.class,
        fairy.getEntityBoundingBox().expand(2.5D, 2.5D, 2.5D));
    ...
    for (int i = 0; i < list.size(); i++) {
      ...
      if (stack != null && !entity1.cannotPickup()) {
        if (stack.getCount() > 0 &&
            goodItem(stack.getItem(), stack.getItemDamage())) {
          list2.add(entity1);
        }
      }
    }
  ...
  public ArrayList<EntityAnimal> getAnimals(final World world) {
    final List<?> list = world.getEntitiesWithinAABB(
        EntityAnimal.class, fairy.getEntityBoundingBox().expand(5D, 5D, 5D));
    ...
    for (int i = 0; i < list.size(); i++) {
      ...
      final int fleeingTick = ReflectionHelper.getPrivateValue(
          EntityCreature.class, entity1, MCP_FLEEINGTICK);
      if (fairy.peacefulAnimal(entity1) && fairy.canEntityBeSeen(entity1) &&
          entity1.getHealth() > 0
          /*&& entity1.getEntityToAttack() == null*/
          && fleeingTick <= 0 && !entity1.isInLove() &&
  ...
  public ArrayList<EntitySheep> getSheep(final World world) {
    final List<?> list = world.getEntitiesWithinAABB(
        EntitySheep.class, fairy.getEntityBoundingBox().expand(5D, 5D, 5D));
    ...
    for (int i = 0; i < list.size(); i++) {
      ...
      final int fleeingTick = ReflectionHelper.getPrivateValue(
          EntityCreature.class, entity1, MCP_FLEEINGTICK);
      if (fairy.canEntityBeSeen(entity1) &&
          entity1.getHealth() > 0 /*&& entity1.getEntityToAttack() == null*/
          && fleeingTick <= 0 && entity1.getGrowingAge() >= 0 &&
          !entity1.getSheared()) {
        list2.add(entity1);
      }
    }
  ...
  private void getNearbyChest2(final int x, final int y, final int z,
                               final World world) {
    int i, j, k;
  ...
  private void getNearbyChest3(final int x, final int y, final int z,
                               final World world) {
    int i, j, k;
  ...
  private boolean checkChestItem(IInventory chest, int slot, final int x,
    ...
    try {
      ...
      if (isBonemealItem(stack.getItem(), stack.getItemDamage()) &&
          onBonemealUse(stack, x, y - 1, z, world)) {
        return true;
      }
      ...
      if (isSaplingBlock(stack.getItem()) &&
          onSaplingUse(stack, x, y - 1, z, world)) {
        return true;
      }
      ...
      // Breeding
      if (!triedShearing && isShearingItem(stack.getItem()) &&
          onShearingUse(stack, world)) {
        return true;
      }

      // Fishing
      if (isFishingItem(stack.getItem()) &&
          onFishingUse(stack, x, y, z, world)) {
        return true;
      }
      ...
      if (stack != null && stack.getCount() > 0) {
        ...
        if (chestStack == null) {
          ...
          assert stack.getItem() ==
              chestStack.getItem(); // avoid duplication glitch?
          assert stack.getCount() + chestStack.getCount() <
              chestStack.getMaxStackSize();
  ...
  private boolean miscActions(final TileEntityChest chest, final int x,
                              final int y, final int z, final World world) {
  ...
  private void cleanSlot(final TileEntityChest chest, final int p) {
    if (chest.getStackInSlot(p) != null &&
        chest.getStackInSlot(p).getItem() == null) {
      chest.setInventorySlotContents(p, (ItemStack)null);
    }
  }

  // What to do with a hoe
  private boolean onHoeUse(final ItemStack stack, int x, final int y, int z,
                           final World world) {
    for (int a = 0; a < 3; a++) {
...
/*         world.playSoundEffect(x + 0.5D, y + 0.5D, z + 0.5D,
                              ...
                              block.getSoundType().getFrequency() * 0.8F); */
  ...
  private boolean onSeedUse(final ItemStack stack, int x, final int y, int z,
                            final World world) {

    /**
     * This can be a bit messy, so will actually defer cleanup until after
     * release.
     */
  ...
  private boolean onBonemealUse(final ItemStack stack, int x, final int y,
                                int z, final World world) {
    for (int a = 0; a < 3; a++) {
...
/*         world.playSoundEffect(x + 0.5F, y + 0.5F, z + 0.5F,
  ...
  private boolean onAxeUse(final ItemStack stack, int x, final int y, int z,
                           final World world) {
    final int m = x;
    final int n = z;
  ...
  private boolean chopLog(final World world, final int x, final int y,
                          final int z) {
    final BlockPos pos = new BlockPos(x, y, z);
    final IBlockState state = world.getBlockState(pos);
    final Block block = state.getBlock();
  ...
  @Deprecated
  private void additionalAxeUse(final ItemStack stack, int x, final int y,
                                int z, final World world, int recurse) {
  ...
  private boolean onSaplingUse(final ItemStack stack, final int x, final int y,
  ...
  private boolean onBreedingUse(final ItemStack stack, final World world) {
    ...
    for (int i = 0; i < animals.size() && count < 3 && stack.getCount() > 0;
         i++) {
      final EntityAnimal entity = (EntityAnimal)animals.get(i);

      int isBreedingCounter = ReflectionHelper.getPrivateValue(
          EntityAnimal.class, entity, MCP_BREEDING);
      // skip unbreedable animals
      if (!entity.isBreedingItem(stack) // can't breed with this item
          || entity.getGrowingAge() !=
                 0 // is juvenile (negative) or recently proceated (positive)
      ...
      if (fairy.getDistance(entity) < 3F) {
        ReflectionHelper.setPrivateValue(EntityAnimal.class, entity, 600,
                                         MCP_INLOVE);
        count++;
        stack.shrink(1);
      }
      ...
      if (!one_sheep.getSheared()) {
        ...
        one_sheep.entityDropItem(
            new ItemStack(Item.getItemFromBlock(Blocks.WOOL), 1,
                          one_sheep.getFleeceColor().getMetadata()),
            0.0F);
  ...
  private boolean onFishingUse(final ItemStack stack, final int x, final int y,
                               final int z, final World world) {
    ...
    final float angle =
        fairy.rotationYaw - 30F + (fairy.getRNG().nextFloat() * 60F);
    ...
    for (int j = -4; j < 0; j++) {
      if (b + j > 0 && b + j < world.getHeight() - 10) {
        ...
        if (!flag) {
          // final Path doug = world.getEntityPathToXYZ( fairy, a, b + j,
          // c, 16F, false, false, true, true );
          final Path path = fairy.getNavigator().getPathToXYZ(a, b + j, c);

          if (path != null && canSeeToSpot(posX, posY, posZ, world)) {
            fairy.rotationYaw = angle;
            fairy.castRod();
            // TODO: player fishing normally damages the rod when the reels
            // something in; should we do that?
            stack.damageItem(1, fairy);
  ...
  private void getToLand(final int x, final int y, final int z,
                         final World world) {
    for (int q = 0; q < 16; q++) {
      ...
      if (y > 1 && y < world.getHeight() - 1) {
        if (fairy.isAirySpace(i, j, k) && !fairy.isAirySpace(i, j - 1, k) &&
            world.getBlockState(new BlockPos(i, j - 1, k))
                .isNormalCube()) {
          // final Path doug = world.getEntityPathToXYZ( fairy, i, j, k,
          // 16F, false, false, true, true );
          final Path path = fairy.getNavigator().getPathToXYZ(i, j, k);
  ...
  private boolean canSeeToSpot(final double posX, final double posY,
                               final double posZ, final World world) {
    return world.rayTraceBlocks(new Vec3d(fairy.posX,
                                          fairy.posY + fairy.getEyeHeight(),
                                          fairy.posZ),
                                new Vec3d(posX, posY, posZ)) == null;
  }

  // Check if it's a good place to put a sapling down
  @SuppressWarnings("unused")
  @Deprecated
  private int goodPlaceForTrees(final int x, final int y, final int z,
                                final World world) {
    final BlockPos pos = new BlockPos(x, y, z);
    final Block i = world.getBlockState(pos).getBlock();
  ...
  /* TODO: I think we can do somewhat better than this, something like
   * Block::harvestBlock for mobs? check inventory for best tool?  can earn
   * harvest?  Can mine ores?
   */
...
/*         world.playSoundEffect(x + 0.5F, y + 0.5F, z + 0.5F,
  ...
  private boolean collectGoodies(final TileEntityChest chest,
                                 final World world) {
    int count = 0;
 ...
 /*      world.playSoundAtEntity(
          fairy, "random.pop", 0.4F,
          ((fairy.getRNG().nextFloat() - fairy.getRNG().nextFloat()) * 0.7F +
           1.0F) *
              2.0F); */
      fairy.armSwing(!fairy.didSwing);
      fairy.attackTime = 1;
  ...
  private int getEmptySpace(final TileEntityChest chest,
    ...
    for (int i = 0; i < chest.getSizeInventory(); i++) {
      ...
      if (temp < 0 && (stack2 == null || stack2.getCount() == 0)) {
        temp = i;
                 ...
                 stack2.getCount() > 0 &&
                 stack2.getCount() + stack.getCount() <=
                     stack.getMaxStackSize() &&
                 !stack2.isItemDamaged() && !stack.isItemDamaged() &&
  ...
  private boolean isHoeItem(final Item item) {
    return item == Items.WOODEN_HOE || item == Items.STONE_HOE ||
        item == Items.IRON_HOE || item == Items.DIAMOND_HOE ||
  ...
  private boolean isAxeItem(final Item item) {
    if (item == Items.WOODEN_AXE || item == Items.STONE_AXE ||
        item == Items.IRON_AXE || item == Items.DIAMOND_AXE ||
  ...
  private boolean breakablePlant(final IBlockState state, final Block above,
                                 final Block below) {
    // we're gonna treat this as everything block that should be punched.
    // cocoa?... hrmmm
    // mushrooms: tricky, when there are at least 4 other mushrooms of same type
    // in 9x3x9 area. snow?  maybe?  if there's plants?  if there's no shovel?
    ...
    return (block instanceof BlockCrops && state.getValue(BlockCrops.AGE) == 7)
        ...
        // cactus: break only when above sand and below cactus, to prevent
        // losing drops.
        || block == Blocks.CACTUS && above == Blocks.CACTUS &&
               below != Blocks.CACTUS
        // melons/pumkins... always?
        || block == Blocks.MELON_BLOCK ||
        state == Blocks.PUMPKIN
  ...
  private boolean isRawFish(final Item item) { return item == Items.FISH; }
  ...
  private boolean goodItem(final Item item, final int j) {
    return isHoeItem(item) || isSeedItem(item) || isBonemealItem(item, j) ||
        isAxeItem(item) || isSaplingBlock(item) || isLogBlock(item) ||
        fairy.acceptableFoods(item) ||
        /* isBreedingItem( item ) || */ isShearingItem(item) ||
        isClothBlock(item) || isFishingItem(item) || isRawFish(item) ||
        isFlower(item);
  }
