...
public class FairyAIFear extends EntityAIBase {
  ...
  public boolean shouldExecute() {
    ...
    if (entityFear == null) {
      ...
      float dist = this.theFairy.getDistance(entityFear);
  ...
  public void startExecuting() {
    ...
    final Path dest = this.theFairy.roam(this.theFairy.getEntityFear(),
                                         this.theFairy, EntityFairy.PATH_AWAY);
          ...
          if (getEntityFear() != null) {
                  if (getEntityFear().isDead) {
                          ...
                          setEntityFear(null);
                                  ...
                                  && willCower()) {
                          float dist = getDistance(getEntityFear());

                          // Run from entityFear if you can see it and it is
  close. if (dist < FairyConfig.BEHAVIOR_FEAR_RANGE) { Path dest =
  roam(getEntityFear(), this, PATH_AWAY);

                                  if (dest != null) {
                                          // setPathToEntity(dest);
                                          this.getNavigator().setPath(dest,
  this.getAIMoveSpeed()); setCryTime(getCryTime() + 120);
                                  }
                          }
                  }
          }
