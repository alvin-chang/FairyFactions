...
public class FairyEntityFishHook extends Entity {
  ...
  // TODO: Fix this
  @SuppressWarnings("unused") @Deprecated private float yOffset;

  /** the number of ticks remaining until this fish can no longer be caught */
  private int ticksCatchable;

  public boolean gotBite() {
    if ((ticksCatchable > 0 || onGround || ticksInAir > 1200) &&
        rand.nextInt(2) == 0) {
  ...
  public FairyEntityFishHook(World par1World, double par2, double par4,
                             double par6) {
    this(par1World);
  ...
  public FairyEntityFishHook(World par1World, EntityFairy fairy) {
    ...
    this.setSize(0.25F, 0.25F);
    this.setLocationAndAngles(fairy.posX,
                              fairy.posY + 1.62D - fairy.getYOffset(),
                              fairy.posZ, fairy.rotationYaw, 0.25F);
    this.posX -=
        (double)(MathHelper.cos(this.rotationYaw / 180.0F * (float)Math.PI) *
                 0.16F);
    this.posY -= 0.10000000149011612D;
    this.posZ -=
        (double)(MathHelper.sin(this.rotationYaw / 180.0F * (float)Math.PI) *
                 0.16F);
    ...
    this.motionY =
        (double)(-MathHelper.sin(this.rotationPitch / 180.0F * (float)Math.PI) *
                 var3);
    this.calculateVelocity(this.motionX, this.motionY, this.motionZ, 0.75F,
                           1.0F);
  }
  ...
  public void calculateVelocity(double par1, double par3, double par5,
    ...
    float var9 =
        MathHelper.sqrt(par1 * par1 + par3 * par3 + par5 * par5);
    par1 /= (double)var9;
    par3 /= (double)var9;
    par5 /= (double)var9;
    ...
    this.prevRotationYaw = this.rotationYaw =
        (float)(Math.atan2(par1, par5) * 180.0D / Math.PI);
    this.prevRotationPitch = this.rotationPitch =
        (float)(Math.atan2(par3, (double)var10) * 180.0D / Math.PI);
  ...
  public void onUpdate() {
    if (getRidingEntity() != null && getRidingEntity() instanceof
                                    EntityFairy) // Client sided only
    {
      getRidingEntity().removePassengers();
      ((EntityFairy)getRidingEntity()).fishEntity = this;
      this.angler = (EntityFairy)getRidingEntity();
    ...
    if (this.fishPosRotationIncrements > 0) {
      double var21 = this.posX + (this.fishX - this.posX) /
                                     (double)this.fishPosRotationIncrements;
      double var22 = this.posY + (this.fishY - this.posY) /
                                     (double)this.fishPosRotationIncrements;
      double var23 = this.posZ + (this.fishZ - this.posZ) /
                                     (double)this.fishPosRotationIncrements;
      double var7;

      for (var7 = this.fishYaw - (double)this.rotationYaw; var7 < -180.0D;
           var7 += 360.0D) {
        ;
      }
      ...
      this.rotationYaw = (float)((double)this.rotationYaw +
                                 var7 / (double)this.fishPosRotationIncrements);
      this.rotationPitch =
          (float)((double)this.rotationPitch +
      ...
      if (!this.world.isRemote) {
        if (this.angler == null || !this.angler.isEntityAlive() ||
            ...
            this.angler.getEntityFear() != null || !this.angler.posted() ||
            !this.angler.isTamed() || !this.angler.isSitting() ||
        ...
        if (this.bobber != null) {
          if (!this.bobber.isDead) {
            ...
            this.posY = this.bobber.getEntityBoundingBox().minY +
                        (double)this.bobber.height * 0.8D;
      ...
      if (this.inGround) {
        Block var19 =
            this.world
                .getBlockState(new BlockPos(this.xTile, this.yTile, this.zTile))
                .getBlock();
      ...
      Vec3d var2 = new Vec3d(this.posX + this.motionX, this.posY + this.motionY,
                             this.posZ + this.motionZ);
      RayTraceResult var3 = this.world.rayTraceBlocks(var20, var2);
      var20 = new Vec3d(this.posX, this.posY, this.posZ);
      var2 = new Vec3d(this.posX + this.motionX, this.posY + this.motionY,
                       this.posZ + this.motionZ);

      if (var3 != null) {
        var2 = new Vec3d(var3.hitVec.x, var3.hitVec.y,
                         var3.hitVec.z);
      }

      Entity var4 = null;
      List<?> var5 = this.world.getEntitiesWithinAABBExcludingEntity(
      ...
      for (int var8 = 0; var8 < var5.size(); ++var8) {
        ...
        if (var9.canBeCollidedWith() &&
            (var9 != this.angler || this.ticksInAir >= 5)) {
          float var10 = 0.3F;
          AxisAlignedBB var11 = var9.getEntityBoundingBox().expand(
      ...
      if (!this.inGround) {
        ...
        float var24 = MathHelper.sqrt(this.motionX * this.motionX +
                                             this.motionZ * this.motionZ);
        this.rotationYaw =
            (float)(Math.atan2(this.motionX, this.motionZ) * 180.0D / Math.PI);

        for (this.rotationPitch =
                 (float)(Math.atan2(this.motionY, (double)var24) * 180.0D /
                         Math.PI);
        ...
        for (int var29 = 0; var29 < var27; ++var29) {
          ...
          double var14 =
              bb.minY +
              (bb.maxY - bb.minY) * (double)(var29 + 0) / (double)var27 -
              0.125D + 0.125D;
          double var16 =
              bb.minY +
              (bb.maxY - bb.minY) * (double)(var29 + 1) / (double)var27 -
              0.125D + 0.125D;
          AxisAlignedBB var18 = new AxisAlignedBB(bb.minX, var14, bb.minZ,
                                                  bb.maxX, var16, bb.maxZ);

          if (this.world.containsAnyLiquid(var18)) {
      ...
      /*       if (this.world.canLightningStrike(pos)) {
              var28 = 300;
            } */
            ...
            /*   this.world.playSoundAtEntity(
                  this, "random.splash", 0.25F,
                  1.0F +
                      (this.rand.nextFloat() - this.rand.nextFloat()) * 0.4F); */
              float var30 = (float)MathHelper.floor(
              ...
              for (var15 = 0; (float)var15 < 1.0F + this.width * 20.0F;
                ...
                this.world.spawnParticle(
                    EnumParticleTypes.WATER_BUBBLE, this.posX + (double)var31,
                    (double)(var30 + 1.0F), this.posZ + (double)var17,
                    this.motionX,
                    this.motionY - (double)(this.rand.nextFloat() * 0.2F),
                    this.motionZ);
              }
              ...
              for (var15 = 0; (float)var15 < 1.0F + this.width * 20.0F;
                ...
                var17 = (this.rand.nextFloat() * 2.0F - 1.0F) * this.width;
                this.world.spawnParticle(
                    EnumParticleTypes.WATER_SPLASH, this.posX + (double)var31,
                    (double)(var30 + 1.0F), this.posZ + (double)var17,
  ...
  public int catchFish() {
    ...
    if (this.bobber != null) {
      ...
      double var8 = (double)MathHelper.sqrt(var2 * var2 + var4 * var4 +
                                                   var6 * var6);
      double var10 = 0.1D;
      this.bobber.motionX += var2 * var10;
      this.bobber.motionY +=
          var4 * var10 + (double)MathHelper.sqrt(var8) * 0.08D;
      this.bobber.motionZ += var6 * var10;
      var1 = 3;
    } else if (this.ticksCatchable > 0) {
      ItemStack fish =
          this.getRandomFishable(this.rand, this.world.rand.nextFloat());
      EntityItem var13 =
          new EntityItem(this.world, this.posX, this.posY, this.posZ, fish);
      double var3 = this.angler.posX - this.posX;
      double var5 = this.angler.posY - this.posY;
      double var7 = this.angler.posZ - this.posZ;
      double var9 = (double)MathHelper.sqrt(var3 * var3 + var5 * var5 +
                                                   var7 * var7);
      double var11 = 0.1D;
      var13.motionX = var3 * var11;
      var13.motionY =
          var5 * var11 + (double)MathHelper.sqrt(var9) * 0.08D;
