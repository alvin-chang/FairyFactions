...
public class EntityFairy extends EntityFlappable implements EntityFlying {
  ...
  public void onUpdate() {
    ...
    if (scout()) {
      this.getEntityAttribute(SharedMonsterAttributes.MOVEMENT_SPEED).setBaseValue(FairyConfig.GENERAL_SPEED_SCOUT);
    } else {
      this.getEntityAttribute(SharedMonsterAttributes.MOVEMENT_SPEED).setBaseValue(FairyConfig.GENERAL_SPEED_BASE);
    }
      ...
      if (world.isRemote) {
        if (++particleCount >= FairyConfig.DEF_MAX_PARTICLES) {
          ...
          if (angry() || (crying() && queen())) {
            ...
            world.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, posX, getEntityBoundingBox().maxY, posZ, 0D, 0D, 0D);
          } else if (crying()) {
            // crying effect
            world.spawnParticle(EnumParticleTypes.WATER_SPLASH, posX, getEntityBoundingBox().maxY, posZ, 0D, 0D, 0D);
          }

          if (liftOff()) {
            // liftoff effect below feet
            world.spawnParticle(EnumParticleTypes.EXPLOSION_NORMAL, posX, getEntityBoundingBox().minY, posZ, 0D, 0D,
                0D);
          }
  ...
  public void despawnFollowers() {
    if (queen() && getFaction() > 0) {
      ...
      for (int j = 0; j < list.size(); j++) {
        ...
        if (fairy != this && fairy.getHealth() > 0 && sameTeam(fairy) && (fairy.ruler == null || fairy.ruler == this)) {
          // TODO: proxy
          fairy.setDead();
  ...
  public Path roamBlocks(double t, double u, double v, Entity actor, float griniscule) {
    ...
    int x = MathHelper.floor(c);
    int y = MathHelper
        .floor(actor.getEntityBoundingBox().minY + (rand.nextFloat() * (u - actor.getEntityBoundingBox().minY)));
    int z = MathHelper.floor(d);
  ...
  private void handleAnger() {
    ...
    if (entityToAttack != null) {
      ...
      if (enemy_dist >= FairyConfig.BEHAVIOR_PURSUE_RANGE || (rand.nextBoolean() && !canEntityBeSeen(entityToAttack))) {
        ++loseInterest;

        if (loseInterest >= (isTamed() ? FairyConfig.BEHAVIOR_AGGRO_TIMER : FairyConfig.BEHAVIOR_AGGRO_TIMER * 3)) {
          setTarget(null);
          loseInterest = 0;
        }
      } else {
  ...
  private void handleRuler() {
    ...
    if (ruler == null) {
      ...
      if (!isTamed() && !queen()) {
        ...
        List<?> list = world.getEntitiesWithinAABB(EntityPlayer.class, getEntityBoundingBox().expand(16D, 16D, 16D));
  ...
  public void doHandleSocial() {
    ...
    for (int j = 0; j < list.size(); j++) {
      ...
      if ((ruler != null || queen()) && entity instanceof EntityFairy && sameTeam((EntityFairy) entity)) {
        ...
        if (creature.getHealth() > 0 && creature.getAttackTarget() != null && creature.getAttackTarget() == ruler) {
  ...
  public static final ItemStack restPotion = PotionUtils.addPotionToItemStack(new ItemStack(Items.POTIONITEM, 1, 16385),
  ...
  private void handleRogue() {
    ...
    for (int j = 0; j < list.size(); j++) {
      ...
      if (canEntityBeSeen(entity) && !entity.isDead) {
        ...
        } else if (ruler != null && canHeal() && entity instanceof EntityCreature && !(entity instanceof EntityCreeper)
            && (!(entity instanceof EntityAnimal) || (!peacefulAnimal((EntityAnimal) entity)))) {
          /**
           * TODO: Update AI.
           *
  ...
  private void handlePosted(boolean flag) {
    ...
    if (postY > -1) {
      ...
      if (ruler != null && (getRidingEntity() == null || getRidingEntity() != ruler) && getDistanceSq(ruler) <= 64F
    ...
    if (getFaction() != 0) {
      if (queen()) {
        ...
        textcomponentstring.appendText(
            getFactionName(getFaction()) + " " + getActualName(getNamePrefix(), getNameSuffix()) + getJob());
      }
    } else if (isTamed()) {
      String woosh = getActualName(getNamePrefix(), getNameSuffix());

      if (queen()) {
        textcomponentstring.appendSibling(new TextComponentTranslation("text.queen.prefix")).appendText(" " + woosh);
      }
  ...
  public boolean processInteract(EntityPlayer player, EnumHand hand) {
    ...
    if (!this.world.isRemote) {
      if (getRidingEntity() == null || getRidingEntity() == player || getRidingEntity() instanceof EntityMinecart) {
        ...
        if (isRuler(player)) {
          if (stack != null && getHealth() < getMaxHealth() && acceptableFoods(stack.getItem())
            ...
            return true;
          } else if (stack != null && getRidingEntity() == null && !isSitting() && vileSubstance(stack.getItem())
              && stack.getCount() > 0) {
  ...
  public boolean vileSubstance(Item i) {
    return i == Items.SLIME_BALL || i == Items.ROTTEN_FLESH || i == Items.SPIDER_EYE || i == Items.FERMENTED_SPIDER_EYE;
  }
  ...
  public void alertRuler(Entity entity) {
    if (getFaction() > 0 && ruler != null && ruler instanceof EntityFairy && sameTeam((EntityFairy) ruler)) {
      ...
      boolean flag = false;
      List<?> list = world.getEntitiesWithinAABB(EntityFairy.class, queen.getEntityBoundingBox().expand(40D, 40D, 40D));
